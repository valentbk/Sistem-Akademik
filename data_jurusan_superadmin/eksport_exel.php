...
//nama file
$nama_file = "Data-Jurursan-" . date('Y-m-d');
// ambil data jurusan dari database
$quey_panggil_jurusan = mysqli_query($db,"SELECT * FROM tbl_jurusan") or die(mysqli_error($db));
...
while ($row = mysqli_fetch_assoc($quey_panggil_jurusan)) {
    ...
    $sheet->setCellValue("B" . $baris, $kode_jurusan);
    $sheet->setCellValue("C" . $baris, $nama_jurusan);
    $baris++;
    $no++;
}
