...
$pdf->Cell(80);
$pdf->Cell(30, 4, 'Data Matkul', 0, 1, 'C');
$pdf->Ln(5);
$pdf->SetFont('Times', '', 12);
$pdf->Cell(15, 6, 'No', 1, 0, 'C');
$pdf->Cell(50, 6, 'Kode Matkul', 1, 0, 'C');
$pdf->Cell(80, 6, 'Nama Matkul', 1, 1, 'C');
$query_panggil_jurusan = mysqli_query($db, "SELECT * FROM tbl_matkul")or die(mysqli_error($db));
$rv = mysqli_num_rows($query_panggil_jurusan);
$no = 1;
if ($rv > 0) {
    while ($data = mysqli_fetch_array($query_panggil_jurusan)) {
        $pdf->Cell(15, 6, $no++, 1, 0, 'C');
        $pdf->Cell(50, 6, $data['kode_matkul'] , 1, 0, 'L');
        $pdf->Cell(80, 6, $data['nama_matkul'], 1, 1, 'C');     
    }
}
$pdf->Output();
?>